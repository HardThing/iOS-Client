...
NSString * const STAdsLayoutAlbumTitleKind = @"headView";
@interface RFQuiltLayout ()
@property(nonatomic) CGPoint firstOpenSpace;
@property(nonatomic) CGPoint furthestBlockPoint;
@property(nonatomic) NSMutableDictionary* headDict;
@property(nonatomic) NSMutableDictionary* layout;
...
- (CGSize)collectionViewContentSize {
    ...
    if (isVert)
        return CGSizeMake(CGRectGetWidth(contentRect), (self.furthestBlockPoint.y+1) * self.blockPixels.height + self.headerHeight);
    else
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (NSIndexPath *indexPath in self.headDict) {
        [attributes addObject:[self layoutAttributesForSupplementaryViewOfKind:STAdsLayoutAlbumTitleKind atIndexPath:indexPath]];
    }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath{
    ...
    if (elementKind == STAdsLayoutAlbumTitleKind) {
        attributes.frame = CGRectMake(0, 0, SCREEN_WIDTH, self.headerHeight);
    }
    return attributes;
}
...
- (void) prepareLayout {
    [super prepareLayout];
    self.headDict = [NSMutableDictionary dictionary];
    if (!self.delegate) return;
    ...
    for (int i = 0; i < [self.collectionView numberOfItemsInSection:0]; i++){
        ...
        if (indexPath.item == 0) {
            UICollectionViewLayoutAttributes *headAttr = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:STAdsLayoutAlbumTitleKind withIndexPath:indexPath];
            headAttr.frame = CGRectMake(0, 0, CGRectGetWidth(self.collectionView.frame), self.headerHeight);
            self.headDict[indexPath] = headAttr;
            self.layout[STAdsLayoutAlbumTitleKind] = self.headDict;
        }
    }
...
- (CGRect) frameForIndexPath:(NSIndexPath*)path {
    ...
    if (isVert) {
        ...
        return CGRectMake(position.x*self.blockPixels.width + initialPaddingForContraintedDimension,
                          position.y*self.blockPixels.height + self.headerHeight,
